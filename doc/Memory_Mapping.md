...
**This kernel writes PSW this way and no other.** The `spl*` routines in
[`kernel/intr.c`](../kernel/intr.c) — БлПр *is* the interrupt priority level — are one `vtm 02003`
and one `vtm 3`, inline from `__besm6_maskpsw`; the four gates in
...
| the u-area | **Two pages, and physical ones**: `u = 074000`, an absolute symbol rather than storage, holding `struct user` (~140 words) with the kernel stack growing up above it to `0100000`. Only the first page (`USIZE` words) is per-process state, and only its live part is copied — everything below `r15`, the count riding in the page as `u_stkdepth` (task 30); the second page is stack overflow that no context switch saves, so a process must not *sleep* in it (task 25a). Being outside every process's map is what forces the copy — see `uflush`/`uload` ([`kernel/uarea.S`](../kernel/uarea.S)). |
