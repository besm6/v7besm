...
- **`cmd/`** — the toolchain as host tools, including `b6sim`, a user-level a.out simulator,
  plus **102 native BESM-6 programs** staged into `build/rootfs/` for the root image — the count
...
`cmd/README.md` §6 is the account. **The root image has 270 free blocks of 2000** — it had 181
...
`/bin/m4` 19, `/bin/make` 24, `/bin/dc` 32 and `/bin/bc` 20 with one more for
`/usr/lib/lib.b`.
