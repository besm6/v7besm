...
The task brief called these two cheap. Four of the six things below were not in any table it
had, and two of them are bugs a user would have met on the first Cyrillic pattern.
...
`MAXSIZ` is 3000, which is 12,000 words; the program measures 17,048 all told. Two
`_Static_assert`s hold it — one on `sizeof(struct words)`, one on the table's word budget — so
...
| `fgrep` | 87 | 3,697 | 212 | 13,052 | **17,048** |

Out of the 28,672 words §6 allows. `fgrep` is the second-largest program on the image after
`fsck`, and every word of the difference is `w[]`.
...
task earlier — the size ctest weighs `const + text + data + bss` and can see neither.
