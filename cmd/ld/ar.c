...
mcmd () {
	...
	if (tf2 < 0) {
		fprintf (stderr,
			MSG ("ar: cannot create third temporary file\n",
...
getaf () {
	...
	if (! getint (af, &mbuf) || mbuf != ARMAG) {
		fprintf (stderr,
			MSG ("ar: %s is not in archive format\n",
...
getqf () {
	...
	if ((qf = open (arnam, 2)) < 0) {
		...
		mbuf = ARMAG;
		if (! putint (qf, mbuf)) wrerr ();
	}
	else
		if (! getint (qf, &mbuf) || mbuf != ARMAG) {
			fprintf (stderr,
				MSG ("ar: %s is not in archive format\n",
					"ar: %s не в формате архива\n"),
				arnam);
...
copyfil (fi, fo, flag) {
	...
	if (flag & HEAD) if (! putarhdr (fo, &arbuf)) wrerr ();
	pe = 0;
...
bamatch () {
	...
	switch (bastate) {
		...
		case 2: 
			...
			if (f < 0) {
				fprintf (stderr,
					MSG ("ar: cannot create second temporary file\n",
