...
TEST(Varargs, MultipleArguments) {
    EXPECT_TOKENS("#define P(...) __VA_ARGS__\nP(a,b,c)\n", "a,b,c");
}

TEST(Varargs, SingleArgument) {
    EXPECT_TOKENS("#define P(...) __VA_ARGS__\nP(x)\n", "x");
}

// A named parameter alongside the variadic part.
TEST(Varargs, NamedPlusVariadic) {
    EXPECT_TOKENS(
...
TEST(Varargs, VaArgsOutsideVariadicDiagnosed) {
