...
void setup(int argc, char *argv[])
{
            ...
            for (;;) {
                ...
                case IDENTIFIER:
                    if ((t = chfind(1, tokname)) < NTBASE) {
                        // cppcheck-suppress arrayIndexOutOfBoundsCond ; a symbol below NTBASE is a
                        // token, bounded by NTERMS
                        j = TYPE(toklev[t]);
                        if (j != 0 && j != ty) {
    ...
    // The prototypes the skeleton needs.  They cannot live in yaccpar.c, which
    // is appended after the user's own subroutines.  README.md, "The contract".
    ...
    *mem++    = 0;
    ...
    more_rule:
        while (t == IDENTIFIER) {
            *mem = chfind(1, tokname);
            if (*mem < NTBASE)
                // cppcheck-suppress arrayIndexOutOfBoundsCond ; likewise
        ...
        if (t == PREC) {
            ...
            // cppcheck-suppress arrayIndexOutOfBoundsCond ; error() above exits, fatfl being on
            // until cpres()
...
static void finact(void)
{
    // finish action routine.  others() rewinds this stream and copies it into
    // y.tab.c, so flush it here rather than closing it -- it was closed and
    // reopened by name when it still had one
...
static int defin(int t, const char *s)
{
    // error() RETURNS when fatfl is off, so the diagnostic arms below are not
    // dead ends and val really can reach the store unset
    int val = 0;
...
int fdtype(int t)
{
    ...
    if (v <= 0) {
        // cppcheck-suppress arrayIndexOutOfBoundsCond ; likewise
        const char *name = (t >= NTBASE) ? nontrst[t - NTBASE].name : tokset[t].name;
...
static void cpyunion(void)
{
    ...
    for (;;) {
        ...
        // cppcheck-suppress invalidFunctionArg ; error() above exits, fatfl being on until cpres()
        putc(c, ftable);
        if (fdefine)
            putc(c, fdefine);
...
static int skipcom(void)
{
    int i = 0; // i is the number of lines skipped
