...
const char *find_parser(void)
{
    ...
    const char *override = getenv("B6YACCPAR");
...
#ifdef besm6
    snprintf(path, sizeof(path), "/usr/lib/yaccpar");
#else
    const char *home = getenv("HOME");

    if (home != NULL && *home != '\0') {
        snprintf(path, sizeof(path), "%s/.local/share/besm6/yaccpar.c", home);
        if (access(path, R_OK) == 0)
            return path;
    }
    snprintf(path, sizeof(path), "/usr/local/share/besm6/yaccpar.c");
#endif
    return path;
}
    ...
    while ((c = getc(finput)) != EOF) {
        if (c == '$') {
            if ((c = getc(finput)) != 'A')
                ...
                // finact() flushed and left it open; rewind rather than reopen.
                // The stream is closed behind the marker, so a second marker is
                // a diagnostic rather than a null FILE *.
                if (faction == NULL)
                    error("parser %s has more than one action marker", parser);
                rewind(faction);
        ...
        // the getc()s above bypass the loop's test, so a skeleton ending in a
        // marker would have written EOF out as a byte
        if (c == EOF)
            break;
        putc(c, ftable);
    }
...
char *writem(int *pp)
{
    ...
    if ((i = *pp) < 0) {
        // an item calling for a reduction; the bound above leaves 30 chars
        q = chcopy(q, "    (");
        snprintf(q, (size_t)(&sarr[ISIZE] - q), "%d)", -i);
    }
...
static void summary(void)
{
    if (foutput != NULL) {
        ...
        fprintf(foutput, "%d shift/reduce, %d reduce/reduce conflicts reported\n", zzsrconf,
                zzrrconf);
        // a pointer difference is one word here and a long on the build machine,
        // so it is cast rather than given an %ld (../README.md SS3)
        fprintf(foutput, "%d/%d working sets used\n", (int)(zzcwp - wsets), WSETSIZE);
        fprintf(foutput, "memory: states,etc. %d/%d, parser %d/%d\n", (int)(zzmemsz - mem0),
                MEMSIZE, (int)(memp - amem), ACTSIZE);
    ...
    // callopt() reads ftemp back, so flush rather than close
    fflush(ftemp);
    if (fdefine != NULL)
        fclose(fdefine);
}
...
int setunion(int *a, const int *b)
{
...
void prlook(const struct looksets *p)
{
    const int *pp = p->lset;
...
static void cpfir(void)
{
    ...
    NTLOOP(i)
    {
        ...
        for (s = pres[i]; s < t; ++s) {
            ...
            for (p = *s; (ch = *p) > 0; ++p) {
                if (ch < NTBASE) {
                    // a symbol below NTBASE is a token, bounded by NTERMS in defin()
...
int state(int c)
{
    ...
    // cppcheck-suppress arrayIndexOutOfBoundsCond ; c < NTBASE is a token, bounded by NTERMS
        ...
        WSLOOP(wsets, u)
        {
            ...
            WSLOOP(u, v)
            {
                if (v->flag == 1 && *(pi = v->pitem) == c) {
                    ...
                    while ((ch = *++pi) > 0) {
                        if (ch < NTBASE) {
                            // terminal symbol; the bound is ntokens, as in cpfir() above
                            // cppcheck-suppress arrayIndexOutOfBoundsCond
