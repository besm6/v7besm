...
// The optimizer reuses the generator's arrays under the names this pass thinks
// in.  Each of these five is the same array, merely renamed.
#define a      amem
#define mem    mem0
#define pa     indgo
#define yypact temp1
...
// These three are arrays of their own.  v7 pointed them into the middle of three
// unrelated tables and walked the dead storage as flat ints, which assumes a
// pointer and an int are the same width -- ../README.md SS2's shape exactly.
// README.md, "y4's borrowed arrays".
static int ggreed[NNONTERM + 2];
static int pgo[NNONTERM + 2];
static int yypgo[NNONTERM + 2];
...
static void arout(const char *s, const int *v, int n);
...
void callopt(void)
{
    ...
    // read the arrays back from the scratch stream summary() flushed.  gtnm()
    // reads through finput, so point that at it
