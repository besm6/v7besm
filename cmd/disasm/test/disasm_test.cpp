...
// address.  An index/modifier register sits in bits 20-23 (reg << 20) and is
// printed ahead of the mnemonic, in a fixed 3-char column -- hence the leading
// spaces on every unindexed line.  The expected text uses the MADLEN dialect
// (the default), whose names match the assembler's table[] in cmd/as/tables.c;
// opcodes the assembler does not name disassemble to the raw "$NN" (short) /
// "@NN" (long) forms it accepts back.
TEST(Disassemble, AllInstructions)
{
    ...
    EXPECT_EQ(decode((5u << 20) | 00120000 | 0123), " 5 aex 0123"); // indexed
    ...
    EXPECT_EQ(decode((4u << 20) | 00270000 | 0567), " 4 xtr 0567"); // indexed
    ...
    EXPECT_EQ(decode((1u << 20) | 00440000 | 6), " 1 mtj 6"); // indexed
    EXPECT_EQ(decode((8u << 20) | 00450000 | 7), " 8 j+m 7"); // indexed
    ...
    EXPECT_EQ(decode((2u << 20) | 02400000 | 040000), " 2 vtm 040000"); // indexed
    ...
    // A register with a zero address: the register alone, no "0" operand.
    // Two-digit registers fill the column exactly.
    EXPECT_EQ(decode((12u << 20) | 00100000), "12 xta");
    EXPECT_EQ(decode((15u << 20) | 00170000), "15 a*x");
