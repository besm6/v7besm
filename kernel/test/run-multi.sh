...
#                /dev/tty0 stays 0/0: a separate inode with no getty on it.
#
#   /etc/utmp    two live records rather than login's one.  A struct member of type char occupies
#                a whole word here, so the record is not a layout any host tool would parse -- but
#                chars pack six to a word with no gaps, so the names themselves are contiguous
#                bytes and can simply be looked for.
#
...
#
# NOTHING IS GRAFTED ONTO THE IMAGE.  Like login, this test types every character it needs,
# because what it is testing is the thing that reads what is typed.
set -e
b6fsutil=$1
besm6=$2
srcdir=$3
...
# run-login.sh's projection of `b6fsutil -v -v': type, mode, uid/gid and the path, with the size
# and i-number dropped because neither is on trial.  Sorted on the path so the expectation does
# not encode the order of the walk.
