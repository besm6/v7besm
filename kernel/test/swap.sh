...
:
: THE V7 SHELL HAS NO COMMENT CHARACTER.  A colon is the null built-in standing in for one,
: but its words are still PARSED -- so no backquote, no apostrophe, NO PARENTHESIS, no
: dollar, no semicolon and no redirection may appear even in a line like this one.  Three
: files in this directory have learned that -- libtest.sh learned it again the day puret
: was added to it, and this one learned it from a SEMICOLON, which runs the rest of the
: line as a command.
:
