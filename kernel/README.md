...
This file is the **reference**: the design the port settled on, the hardware rules that design
obeys, and the facts that cost real time to establish. Anything narrower belongs in the source
comments and in [../doc/](../doc/) — read [Memory_Mapping.md](../doc/Memory_Mapping.md) before
touching memory management, [Intrinsics.md](../doc/Intrinsics.md) for how C reaches `002 «рег»` and
`033 «увв»`, [Unix_Context_Switch.md](../doc/Unix_Context_Switch.md) for the gates,
[Kernel_Assembly_Routines.md](../doc/Kernel_Assembly_Routines.md) for the machine assist, and
[Besm6_Peripherals.md](../doc/Besm6_Peripherals.md) before touching [dev/](dev/).
...
takes `-lruntime` **alone** — the kernel has its own `printf` in [prf.c](prf.c) and calls no
...
once in [uarea.S](uarea.S). The page above the saved one is stack **overflow**: the stack may grow
...
[unix.ini](unix.ini) has the *simulator* load one off the build host. So [kctl.c](kctl.c) carries a
table of the variables the kernel publishes and `kctl(2)` reads it (`<sys/kctl.h>`,
[../doc/Unix_V7_System_Calls.md](../doc/Unix_V7_System_Calls.md) §2.5). It costs about **390 words**
...
([../doc/Besm6_Data_Representation.md](../doc/Besm6_Data_Representation.md) §7).
...
the foot of [kctl.c](kctl.c) with one record on the kernel stack and a `copyout` per slot — **no
...
words 0–7 — see `getxfile()` in [sys1.c](sys1.c).)
...
3. **Drain the БРЗ write cache** — nine consecutive stores to physical 1–7 with mapping off.
   `drainbrz()` is the one routine in the kernel that **has to be assembly** ([brz.s](brz.s));
...
handed out. `resume()` ([switch.s](switch.s)): if `paddr != uhome`, `uflush(uhome)`, then
...
a stack that does not exist. That is what `SLACK` in [uarea.S](uarea.S) is for.
...
* **`DIRSIZ` moves `u_upt`.** `struct user` holds `u_dbuf[DIRSIZ]` and a `struct direct` ahead of
  the shadow page table, whose word offset [uarea.S](uarea.S), [seg.S](seg.S) and `test/mmutest.c`
...
* **The dead tail of the live u-area belongs to whoever ran before.** `uload()` writes only the live
  ...
  `core()`, which dumps the whole `USIZE` page ([sig.c](sig.c)), and `ptrace`'s u-area window can
...
* **`copyin`/`copyout` toggle БлП per word** (~2× a plain copy) and are **word-only on purpose** —
  ...
  offsets are peeled a level up, in `copyinb`/`copyoutb` ([ucopy.c](ucopy.c)) — align the
...
* **The tick is four times v7's, and two things are scaled for it by hand.** `HZ` is 250 because the
  ...
  [clock.c](clock.c)) because its decay is per *second* and cannot move, and `CLOCKS_PER_SEC` in
  `<time.h>` is a hand-copy of `HZ` that `lib/libc/gen/clock.c` `_Static_assert`s. One consequence
  is **left unfixed**: `acct(2)`'s `compress()` ([acct.c](acct.c)) has a 13-bit mantissa, so a CPU
...
* **`profil(2)` is refused and `addupc()` does not exist.** The gate is real — v7's four arguments,
  `EINVAL` for any scale but the 0 or 1 v7 itself reads as "profiling off" ([sys4.c](sys4.c)) — but
...
* **`ptrace`'s single-step, request 9, is refused with `EIO`.** v7 sets the PDP-11 T-bit; this
  machine has only a pair of debug registers ([../doc/Memory_Mapping.md](../doc/Memory_Mapping.md)
  ...
  ported that would use it. The refusal is `procxmt()`'s ordinary one ([sig.c](sig.c)), so the child
