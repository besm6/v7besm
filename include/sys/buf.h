...
struct buf {
    ...
    // Core address of the block's data, a WORD pointer -- deliberately not a caddr_t.
    // A fat pointer's bit-48 marker reads as a floating exponent when b6cc adds a field
    // offset, so a struct read off one silently returns word 0 unless explicitly cast;
    // a word pointer has nothing to strip.  The two byte-granular users -- readi()/writei()
    // with `+ on', and exec()'s arglist staging -- say `(caddr_t)bp->b_addr', which lands
    // on the word's first byte (doc/Besm6_Data_Representation.md §7).
    int *b_addr;

    // Where the data lives, physically, in WORDS -- valid only when B_PHYS.
    // b_addr cannot serve: a C pointer's word field is 15 bits, so it cannot
    // name anything above 32767, and physical memory runs to 512 Kwords.
    // Filled by swap() and physio(); read through bufpaddr().
    paddr_t b_paddr;
};
...
//
// A kernel buffer is unmapped, so its address IS its physical address and b_addr's word
// field is the whole answer.  A B_PHYS request carries the address explicitly, because
// its target may be above the 32767 that field can reach.
